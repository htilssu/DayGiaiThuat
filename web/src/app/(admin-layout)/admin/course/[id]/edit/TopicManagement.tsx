...
import { getAllTopicsAdmin, assignTopicToCourseAdmin, type AdminTopic } from '@/lib/api/admin-topics';
...
export default function TopicManagement({ courseId, opened, onClose }: TopicManagementProps) {
    ...
    React.useEffect(() => {
        if (topics.length > 0 && courseId) {
            const courseTopics = topics.filter((topic: AdminTopic) => topic.courseId === courseId);
            setSelectedTopics(new Set(courseTopics.map((topic: AdminTopic) => topic.id)));
        }
    }, [topics, courseId]);
