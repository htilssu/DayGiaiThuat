...
export default function CourseDetailPage() {
  ...
  useEffect(() => {
    if (courseData) {
      ...
      if (courseData.isEnrolled && courseData.topics && courseData.topics.length > 0) {
        const totalLessons = courseData.topics.reduce((total, topic) => total + (topic.lessons?.length || 0), 0);
        const completedLessons = courseData.topics.reduce((total, topic) => 
          total + (topic.lessons?.filter(lesson => lesson.isCompleted).length || 0), 0);
        
        setAllLessonsCompleted(totalLessons > 0 && completedLessons === totalLessons);
      } else {
