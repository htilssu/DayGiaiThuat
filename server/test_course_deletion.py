...
def test_course_cascade_delete():
    ...
    try:
        ...
        print(f"Testing deletion for Course ID: {course_id} - '{course_with_content.title}'")
        
        # Đếm số lượng topics, lessons, sections trước khi xóa
        topics_before = db.query(Topic).filter(Topic.course_id == course_id).count()
        lessons_before = db.query(Lesson).join(Topic).filter(Topic.course_id == course_id).count()
        sections_before = db.query(LessonSection).join(Lesson).join(Topic).filter(Topic.course_id == course_id).count()
        ...
        lessons_after = db.query(Lesson).join(Topic).filter(Topic.course_id == course_id).count()
        sections_after = db.query(LessonSection).join(Lesson).join(Topic).filter(Topic.course_id == course_id).count()
